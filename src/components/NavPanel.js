...
export default function NavPanel(props) {
    ...
    const navMenu = navMenuItems.map((i, key) => {
        return (
            <ul className={`${i.group}`} key={key + 1}>
                {i.names.map((g, key) => { return <li className={`Nav-menu-item ${props.state.navOpen ? 'Open' : 'Closed'} ${g.name}`} key={key + 2} onClick={() => closeNav()} aria-label={`${g.ariaLabel}`}></li> })}
            </ul>
        )
    })
    ...
    return (
        <React.Fragment>
            <div
                className={`Nav-panel-container ${props.state.navOpen ? 'Open' : 'Closed'}`}
                ...
                <div className={`Nav-panel ${props.state.navOpen ? 'Open' : 'Closed'}`}>
                    ...
                    <svg className='Nav-panel-svg-logo' width="2009" height="976" viewBox="0 0 2009 976" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <line x1="2009" y1="755.5" x2="9.00098" y2="755.5" stroke="#BAE9DD" strokeWidth="200" />
                ...
                <div className={`Close-click-area ${props.state.navOpen ? 'Open' : 'Closed'}`} onClick={() => closeNav()}></div>
            </div>
            <div className={`Floating-nav-button ${props.state.navOpen ? 'Open' : 'Closed'}`} onClick={() => openNav()}>
