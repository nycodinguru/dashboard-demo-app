...
export default function NavPanel() {
    ...
    function openNav() { 
        setNavPanelOpen({...navPanelOpen, isOpen: true })
        document.querySelector('.Dashboard-container').classList.add('Open')
        document.querySelector('body').classList.add('Open')
    }

    function closeNav() {
        setNavPanelOpen({...navPanelOpen, isOpen: false })
        setTimeout(() => {
    ...
    const navMenu = navMenuItems.map((i, key) => {
        return (
            <ul className={`${i.group}`} key={key + 1}>
                {i.names.map((g, key) => { return <li className={`Nav-menu-item ${navPanelOpen.isOpen ? 'Open' : 'Closed'} ${g.name}`} key={key + 2} onClick={() => closeNav()} aria-label={`${g.ariaLabel}`}></li> })}
            </ul>
        )
    })
    ...
    return (
        <React.Fragment>
            <div
                ...
                <div className={`Close-click-area ${navPanelOpen.isOpen ? 'Open' : 'Closed'}`} onClick={() => closeNav()}></div>
            </div>
            <div className={`Floating-nav-button ${navPanelOpen.isOpen ? 'Open' : 'Closed'}`} onClick={() => openNav()}>
